...
public class DiskLruCache {
	...
	private static final String CACHE_FILENAME_PREFIX = "cache_"; // 缓存文件名前缀
	private static final int MAX_REMOVALS = 4; // 最大屏蔽数？
		...
		synchronized (mLinkedHashMap) {
			if (mLinkedHashMap.get(key) == null) {
				try {
					...
					Log.e(TAG, "put操作时发生错误: " + e.getMessage());
	...
	private void flushCache() {
		...
		while (count < MAX_REMOVALS
			...
			if (BuildConfig.DEBUG) {
				Log.d(TAG, "flushCache - 移除缓存文件, " + eldestFile + ", "
						+ eldestFileSize);
			}
		}
	}
	...
	/**
	 * 获取一个可用的缓存目录（如果可用则使用外部储存，否则使用内部储存：context.getCacheDir().getPath()）
	 * 
	 * @param context
	 * @param uniqueName
	 *            一个唯一名字的缓存目录
	...
	public static File getDiskCacheDir(Context context, String uniqueName) {
		...
		// 检测media是否是mounted或者storage是内置的，如果是的话则使用外部存储目录，否则使用内部缓存目录（内存）
		final String cachePath =
		// 此处是demo原来的写法，比较方式有问题，应该用equals来比较
